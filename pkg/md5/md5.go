...
//Added by Roger Coll
func MakePaddingBlock(input []byte, messlength int) []byte {
	left := len(input) % 64
	if left > 56 {
		left = 56 + (64 - left)
	} else {
	...
	pb := make([]byte, len(input)+left)
	for i := 0; i < len(input); i++ {
		pb[i] = input[i]
	}
	pb[len(input)] = pb[len(input)] | (1 << 7)

	//now we must add the last 64 bits => mess length
	mlength := make([]byte, 4)
	binary.LittleEndian.PutUint32(mlength, 8*uint32(messlength))
	pb = append(pb, mlength...)
	pb = append(pb, 0x0, 0x0, 0x0, 0x0)
	return pb
}

func PreviousToSum(data []byte) digest {
	var d digest
	d.Reset()
	uints := Bytes2Uints(data)
	d.s[0] = uints[0]
	d.s[1] = uints[1]
	d.s[2] = uints[2]
	d.s[3] = uints[3]
...
func (d *digest) ForgedCheckSum() [Size]byte {
	//don't look try to append the PB as we already created the block
	var digest [Size]byte
	binary.LittleEndian.PutUint32(digest[0:], d.s[0])
	binary.LittleEndian.PutUint32(digest[4:], d.s[1])
	binary.LittleEndian.PutUint32(digest[8:], d.s[2])
